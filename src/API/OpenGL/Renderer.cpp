...
namespace OpenGLRenderer {
	...
	std::vector<PointLight> g_sceneLights = {};
	...
	void Init() {
		...
		//g_sceneLights.push_back(cubeLampLight);
	...
	void RenderFrame() {
		...
		//const int FPS_SMOOTHING_SAMPLES = 50;
		//double fpsBuffer[FPS_SMOOTHING_SAMPLES] = { 0 };
		//int fpsIndex = 0;

		//if (g_deltaTime > 0.0) {
		//	double currentFPS = 1.0 / g_deltaTime;
		...
		//	g_fps = fpsSum / FPS_SMOOTHING_SAMPLES;
		//}

		//glm::mat4 view = glm::mat4(1.0f);
		//glm::mat4 projection = glm::mat4(1.0f);

		//view = Game::GetPLayerByIndex(0).camera.getViewMatrix();
		//projection = glm::perspective(glm::radians(Game::GetPLayerByIndex(0).camera.getZoom()), (float)Window::currentWidth / (float)Window::currentHeight, 0.1f, 100.0f);

		//// ------ SHADOW PASS (Render to Depth Map) ------
		//glm::mat4 orthogonalProjection = glm::ortho(-10.0f, 10.0f, -10.0f, 10.0f, 1.0f, 7.5f);

		//glm::mat4 lightView = glm::lookAt(g_sceneLights[0].position, AssetManager::GetModelByName("Cube")->pos, glm::vec3(0.0f, 1.0f, 0.0f));
